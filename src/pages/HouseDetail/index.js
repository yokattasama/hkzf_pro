...
import { API, BASE_URL, isAuth, removeToken } from '../../utils'
...
export default class HouseDetail extends Component {
  ...
  async getHouseDetail () {
    ...
    // 渲染地图
    const { community, coord: { latitude, longitude} } = res.body
  ...
  async checkCollected () {
    ...
    // console.log(res)
    this.setState({
      isCollected: res.body.isFavorite
    })
  }
  // 添加收藏
  handleCollect = async () => {
    ...
    if(isCollected) {
      ...
      if(res.status === 200) {
        ...
        Toast.info('登录超时，请登陆后重试', 2, null, false)
        removeToken()
      }
    } else {
      // 若没有收藏，则添加收藏
      ...
      if(res.status === 200) {
        ...
        Toast.info('登录过期或超时，请重新登录后再试', 2, null, false)
        removeToken()
      }
    }
  ...
  componentDidMount() {
    // 获取路由参数
    // console.log('路由参数', this.props.match.params)
    this.getHouseDetail()
    this.checkCollected()
  }
  renderTags () {
  ...
  renderSwipers() {
    ...
    return houseImg.map((item, index) => (
      <a key={index} href="http://itcast.cn"
        style={{
  ...
  render() {
    const { isLoading, isCollected, houseInfo: { community, title, price, roomType, size, oriented, floor, description, supporting } } = this.state
    return (
        ...
        <Flex className={styles.fixedBottom}>
          <Flex.Item onClick={this.handleCollect}>
            <img
              ...
              alt="收藏"
            />
            <span className={styles.favorite}>{isCollected ? '已收藏' : '收藏'}</span>
          </Flex.Item>
